...
from utils import (
    psnr, saveModel, saveModelState, 
    savePlot, saveValidationResults
)
    ...
    with torch.no_grad():
        for bi, data in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            if SAVE_VALIDATION_RESULTS and (epoch % 500) == 0:
                saveValidationResults(outputs, epoch, bi)
    final_loss = running_loss/len(dataloader.dataset)
    final_psnr = running_psnr/len(dataloader)
    return final_loss, final_psnr
...
for epoch in range(epochs):
    ...
    # resuming training.
    if (epoch+1) % 100 == 0:
        saveModel(epoch, model, optimizer, criterion)
    ...
    savePlot(train_loss, val_loss, train_psnr, val_psnr)
end = time.time()
